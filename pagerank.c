...
void init_link_matrix(const char *filename, int num_pages, float link_matrix[][num_pages]) {
	...
	for (int i = 0; i < num_pages; i++) {
		...
		do {
			...
			if (fscanf(file, "%d", &num_links) != 1) {
        fprintf(stderr, "Error reading number of links for page\n");
        exit(EXIT_FAILURE);
    }
			if (num_links < 1)
		...
		for (int j = 0; j < num_links; j++) {
			...
			do {
				...
				if (fscanf(file, "%d", &page_num) != 1) {
                fprintf(stderr, "Error reading linked page for page %d\n", i + 1);
                exit(EXIT_FAILURE);
            }
